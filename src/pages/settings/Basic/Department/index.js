...
const Department = () => {
  const columns = [
    ...
    },
    {
      ...
      render: (productGroup) => productGroup.title
    },
    {
      title: "Department Head",
      dataIndex: "head",
      render: (head) => head?.first_name + ' ' + head?.last_name
    },
    {
      title: 'Status',
      dataIndex: 'is_active',
  ...
  const loadData = async (searchVal, pagination) => {
    setLoading(true);
    const employees = await axios.get("/settings/departments", {params: {
      searchVal,
      ...
      onOk: async () => {
        try {
          const res = await axios
            .post(`/settings/departments/block/${rowId}`)
            .then((res) => res.data);
          if (res?.data?.id) {
